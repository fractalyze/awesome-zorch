...
# a ∘ b = c) → lincheck (a = A·z, b = B·z) → batched Ligerito open. `prove_fast`
# fuses all four into one device call; `verify` is the dual — it replays the
# cheap sumcheck stages to recover the opening point, then checks the Ligerito
...
from flock_zorch import prover, verifier  # noqa: E402
...
proof = prover.prove_fast(z_packed, M, K_LOG, K_SKIP, eye, eye,
                          z_lincheck, statement, CFG, domain=DOMAIN)

root = commit_flock_ligerito(CFG, z_packed)[0]  # the commitment the verifier binds to
ok = verifier.verify(CFG, root, statement, proof, eye, eye, M, K_LOG, K_SKIP,
                     Challenger(DOMAIN))
