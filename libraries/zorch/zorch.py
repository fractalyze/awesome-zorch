...
# Bind the round over every variable; stack the round polys into the proof. The
# challenge policy is a soundness parameter: naming the transcript's own field
# draws base-field challenges, an extension dtype raises the soundness floor.
round = prover.StandardRound(Product(degree=DEGREE), challenges=ChallengePolicy(F))
# The carry is the folding state beside the running claim: each round's challenge
# is derived from the transcript, so the point being bound lives nowhere else.
carry = prover.initial_claim(fnp.stack([a, b]), claimed, LOG_N)
_, _, msgs = fold_rounds(round, carry, transcript(), LOG_N)
...
point, final_claim, _, ok = verify(
    verifier.SumcheckRound(DEGREE, ChallengePolicy(F)), claimed, proof, transcript()
)
