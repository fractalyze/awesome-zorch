  ...
  return (
    <header className="sticky top-0 z-40 border-b border-edge bg-bg/85 backdrop-blur-md">
      <div className="mx-auto flex h-16 max-w-6xl items-center gap-8 px-4 sm:px-6">
        ...
        <a
          href={SITE.coreRepo}
          className="ml-auto inline-flex items-center gap-2 rounded-full bg-ink px-4 py-2 text-[13px] font-medium text-bg transition-colors hover:bg-ink/85"
        >
          ...
          <StarIcon className="size-3.5 text-[#e3b341]" />
          <span className="hidden sm:inline">Star</span>
          {coreStars != null && <span className="font-mono text-xs text-black/70">{formatStars(coreStars)}</span>}
        </a>
      </div>
    </header>
  );
